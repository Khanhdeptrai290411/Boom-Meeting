    ...
    switch (activeTab) {
      case 'archive':
        ...
        case 'users':
          return (
            <div className="overflow-y-auto flex-1 p-4">
              <h2 className="text-xl font-semibold dark:text-white mb-4">Friend Requests</h2>
              {/* Lọc và hiển thị những người có status = '0' (chưa được accept) */}
              {chats
                .filter((chat) => chat.status === '0')
                .map((chat) => (
                  <div
                    key={chat.id}
                    className="flex items-center justify-between p-2 bg-gray-100 dark:text-white dark:bg-gray-800 rounded-md mb-2"
                  >
                    <div className="flex items-center">
                      ...
                      <span className="dark:text-white">{chat.name}</span>
                    </div>
